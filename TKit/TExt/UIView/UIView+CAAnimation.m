...
@interface UIViewAnimationDelegate : NSObject

@property (nonatomic, strong) dispatch_block_t doneBlk;
@property (nonatomic, strong) dispatch_block_t startBlk;

+(UIViewAnimationDelegate*) getInstance;

@end

#define     SET_ANIMATION_DELEGATE(animation)         animation.delegate = [UIViewAnimationDelegate getInstance];
#define     SET_UIVIEW_ANIMATION_DELEGATE       [UIView setAnimationDelegate:[UIViewAnimationDelegate getInstance]];\
        ...
        view                :(UIView *)theView
{
     ...
     *
     *  @see CAAnimationDelegate    (按下TCommand键点击)
     */

    animation.delegate = [UIViewAnimationDelegate getInstance];
...
{
    CATransition *animation = [CATransition animation];
    SET_ANIMATION_DELEGATE(animation);
...
{
    SET_UIVIEW_ANIMATION_DELEGATE
    [UIView beginAnimations:nil context:NULL];
    [UIView setAnimationCurve:UIViewAnimationCurveEaseInOut];
    
    [UIView setAnimationDelegate:[UIViewAnimationDelegate getInstance]];
...
+ (void)animationMoveDown:(UIView *)view duration:(CFTimeInterval)duration
{
    ...
    SET_ANIMATION_DELEGATE(transition);
...
+ (void)animationRotateAndScaleDownUp:(UIView *)view
{
    ...
    SET_ANIMATION_DELEGATE(animationGroup);
...
///  使用 [UIView animationxxx]; 之前调用设置 动画完成回调
extern void UIAnimationDone(dispatch_block_t doneBlk)
{
    [UIViewAnimationDelegate getInstance].doneBlk = doneBlk;
}

///  使用 [UIView animationxxx]; 之前调用设置 动画开始回调
extern void UIAnimationStart(dispatch_block_t startBlk)
{
    [UIViewAnimationDelegate getInstance].startBlk = startBlk;
}

@implementation UIViewAnimationDelegate

-(void) setDoneBlk:(dispatch_block_t)doneBlk
{
    if (!doneBlk) {return;}
...
-(void) setStartBlk:(dispatch_block_t)startBlk
{
    if (!startBlk) {return;}
    _startBlk = nil;
    _startBlk = startBlk;
...
static UIViewAnimationDelegate* kInstance;
+(UIViewAnimationDelegate*) getInstance{
    if (!kInstance) {
        kInstance = [[UIViewAnimationDelegate alloc] init];
    }
    return kInstance;
}
